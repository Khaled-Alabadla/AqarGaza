...
@section('page-header')
    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                <h4 class="content-title mb-0 my-auto">تعديل الصلاحيات</h4><span class="text-muted mt-1 tx-13 mr-2 mb-0">/
                    {{ $admin->name }}</span>
            </div>
        </div>
    </div>
        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <form class="form-horizontal" action="{{ route('dashboard.admins.update', $admin->id) }}"
                        method="POST">
                        @method('PUT')
                        @csrf
                                    ...
                                    @foreach ($roles as $role)
                                        <div class="form-check d-flex align-items-center gap-3 mb-3 border-bottom pb-2"
                                            ...
                                            <input class="form-check-input" type="checkbox" id="role_{{ $role->id }}"
                                                name="roles[]" value="{{ $role->id }}" @checked($admin->roles->contains('id', $role->id))>
