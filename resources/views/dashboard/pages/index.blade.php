...
@section('title', 'عرض قائمة الصفحات')
            ...
            <div class="card mg-b-20">
                <div class="card-header pb-0">
                    <div class="d-flex justify-content-between">
                        <h4 class="card-title mg-b-0">الصفحات</h4>
                        @can('pages.create')
                            <a href="{{ route('dashboard.pages.create') }}" class="btn btn-primary">إضافة صفحة جديدة</a>
                        @endcan
                    </div>
                        ...
                        <table id="example" class="table key-buttons text-md-nowrap">
                            <thead>
                                <tr>
                                    ...
                                    <th class="border-bottom-0">اسم الصفحة</th>
                                    <th class="border-bottom-0">العنوان</th>
                                    <th class="border-bottom-0">العنوان الفرعي</th>
                                    @if (auth()->user()->can('pages.update') || auth()->user()->can('pages.delete'))
                                        <th>العمليات</th>
                                    @endif
                                </tr>
                            </thead>
                            <tbody>
                                @foreach ($pages as $page)
                                    <tr>
                                        ...
                                        @if (auth()->user()->can('pages.delete') || auth()->user()->can('pages.update'))
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content modal-content-demo">
                    <div class="modal-header">
                        <h6 class="modal-title">حذف الصفحة</h6><button aria-label="Close" class="close"
...
@section('js')
    ...
    <script>
        $('#modaldemo1').on('show.bs.modal', function(event) {
            ...
            var pageId = button.data('id'); // Extract the ID from data-* attributes
            var modal = $(this);
            modal.find('form').attr('action', '/dashboard/pages/' + pageId);
        });
    </script>
    <script>
