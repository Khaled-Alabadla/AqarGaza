            ...
            <div class="card mg-b-20">
                <div class="card-header pb-0">
                    <div class="d-flex justify-content-between">
                        <h4 class="card-title mg-b-0">المدونة</h4>
                        @can('admins.create')
                            <a href="{{ route('dashboard.blogs.create') }}" class="btn btn-primary">إضافة منشور جديد</a>
                        @endcan
                    </div>
                                            ...
                                            <td class="d-flex" style="gap: 3px">
                                                ...
                                                @can('blogs.update')
                                                    <a href="{{ route('dashboard.blogs.edit', $blog->id) }}"
                                                        class="btn btn-info-gradient btn-sm"> تعديل</i></a>
                                                @endcan
                                                @can('blogs.delete')
...
@section('js')
    ...
    <script>
        $('#modaldemo1').on('show.bs.modal', function(event) {
            ...
            var blogId = button.data('id'); // Extract the ID from data-* attributes
            var modal = $(this);
            modal.find('form').attr('action', '/dashboard/blogs/' + blogId);
        });
    </script>
    <script>
